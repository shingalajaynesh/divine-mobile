...
import { CLEAR_RECENT_CONTENT_SEARCHES_MUTATION, CONTENT_FEED_QUERY, RECENT_CONTENT_SEARCHES_QUERY, SAVED_CONTENT_QUERY, SEARCH_CONTENT_QUERY, SET_CONTENT_BOOKMARK_MUTATION } from '../graphql/operations.js';
...
const views = [['explore', 'Explore'], ['bookmark', 'Saved'], ['watch_later', 'Watch later']];
...
export default function MobileLibrary({ lang = 'en' }) {
  ...
  const saved = useQuery(SAVED_CONTENT_QUERY, { variables: { kind: view, language: lang }, skip: view === 'explore', fetchPolicy: 'cache-and-network' });
  ...
  return <View style={s.screen}><Text style={s.eyebrow}>DIVINE LIBRARY</Text><Text style={s.heading}>Learn, listen and connect</Text><Text style={s.intro}>Search original guidance and keep favourites close.</Text>
    ...
    {loading ? <ActivityIndicator color={colors.maroon} /> : error ? <Text style={s.empty}>Library could not be loaded.</Text> : items?.length ? <View style={s.grid}>{items.map((item) => <View key={item.id} style={s.card}><View style={s.icon}><Ionicons name={item.contentType === 'audio' ? 'headset-outline' : item.contentType === 'video' ? 'play-outline' : 'book-outline'} size={22} color={colors.maroon} /></View><Text style={s.category}>{item.category?.name || item.contentType}</Text><Text style={s.title}>{item.translation?.title}</Text><Text style={s.body} numberOfLines={4}>{item.translation?.summary || item.translation?.body}</Text><Text style={s.access}>{item.visibility === 'free' ? 'Included' : `${item.visibility} access`}</Text><View style={s.actions}>{view === 'explore' ? <><TouchableOpacity disabled={bookmarkState.loading} style={s.action} onPress={() => save(item.id, 'bookmark')}><Ionicons name="bookmark-outline" size={15} color={colors.maroon} /><Text style={s.actionText}>Save</Text></TouchableOpacity>{['video','audio'].includes(item.contentType) ? <TouchableOpacity disabled={bookmarkState.loading} style={s.action} onPress={() => save(item.id, 'watch_later')}><Ionicons name="time-outline" size={15} color={colors.maroon} /><Text style={s.actionText}>Later</Text></TouchableOpacity> : null}</> : <TouchableOpacity disabled={bookmarkState.loading} style={s.removeAction} onPress={() => save(item.id, view, false)}><Text style={s.removeText}>Remove</Text></TouchableOpacity>}</View></View>)}</View> : <Text style={s.empty}>{activeQuery ? `No results for “${activeQuery}”.` : 'No saved content yet.'}</Text>}
  </View>;
}
