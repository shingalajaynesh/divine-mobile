...
import { ENROLL_IN_PROGRAM_MUTATION, MY_PROGRAM_ENROLLMENTS_QUERY, PROGRAM_CATALOG_QUERY } from '../graphql/operations.js';
...
export default function MobileProgrammes() {
  ...
  return <View style={s.screen}>
    ...
    {(data?.programCatalog || []).map((program) => {
      ...
      const activityCount = program.modules.reduce((total, module) => total + module.lessons.reduce((count, lesson) => count + lesson.activities.length, 0), 0);
      return <View key={program.id} style={s.card}>
        ...
        <Text style={s.title}>{program.name}</Text><Text style={s.summary}>{program.summary}</Text><Text style={s.meta}>{program.modules.length} modules · {activityCount} activities</Text>
        ...
        {expanded === program.id ? <View style={s.modules}>{program.modules.map((module) => <View key={module.id} style={s.module}><Text style={s.moduleTitle}>{module.title}</Text><Text style={s.moduleDescription}>{module.description}</Text>{module.lessons.map((lesson) => <View key={lesson.id} style={s.lesson}><View style={s.lessonIcon}><Ionicons name={lesson.lessonType === 'audio' ? 'headset-outline' : 'play-outline'} size={16} color={colors.maroon} /></View><View style={{ flex: 1 }}><Text style={s.lessonTitle}>{lesson.title}</Text><Text style={s.lessonMeta}>{lesson.durationMins || 0} min · {lesson.activities.length} activities</Text></View></View>)}</View>)}</View> : null}
      </View>;
    })}
  </View>;
}
